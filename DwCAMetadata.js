...
function generateDwCA(data){
  ...
  var result = createZenodoDeposition_();
  var packageId = !result ? Utilities.getUuid() : result.metadata.prereserve_doi.doi;

  var eml = createEML_(dwcaFolder, packageId);
  var meta = createMetaXML_(dwcaFolder);
  var csvFiles = createCSVFiles_(dwcaFolder);

  var blobsSource = [];
  var iter = dwcaFolder.getFiles();
  while(iter.hasNext()) {
    var file = iter.next();
  ...
  var dwcArchive = dwcaFolder.createFile(Utilities.zip(blobsSource, 'dwca-' + (new Date().getTime()) + '.zip'));
  if ( dwcArchive ) {
    if ( result && result.id ) {
      ...
      // Download DwCA
      var downloadURL = dwcArchive.getDownloadUrl().slice(0, -8);
      showurl(downloadURL);
    }
  }
}
...
function createMetaXML_(folder) {
  ...
  for (var i = 0; i < mappedSheets.length; i++) {
    ...
    var elem = createMetaElem_(sheet, mappedSheets[i].core, mappedSheets[i].rowType);
  ...
  var iter = folder.getFilesByName('meta.xml');
  if (iter.hasNext()) {
    var file = iter.next();
    file.setContent(xml);
    return file;
...
function createCSVFiles_(folder) {
  ...
  for (var i = 0; i < mappedSheets.length; i++) {
    ...
    rowType = mappedSheets[i].rowType;
    fileName = rowType.substr(rowType.lastIndexOf(':')+1).toLowerCase() + '.csv';
...
function createMetaElem_(sheet, isCore, rowType) {
  var elem =  XmlService.createElement(isCore ? 'core' : 'extension')
  ...
  .setAttribute('rowType', getTermURI(rowType))
...
function getTermURI(term) {
  var props = PropertiesService.getDocumentProperties();
  var namespaces = JSON.parse(props.getProperty('ns'));
  var index = term.lastIndexOf(':');
  if (index >= 0) {
    var prefix = term.substr(0, index);
    return namespaces[prefix] !== undefined ? (namespaces[prefix] + term.substr(index+1)) : null;
  }
  return null;
}
...
function getMetadata(sheet) {
  ...
  var coreSheetIndex = props.getProperty('dwca-core-sheet')
  var sheetKey = sheet.getName().replace(/\s+/gi, '-').toLowerCase();
  var hasMappingRow = props.getProperty(sheetKey + '-dwcheader');

  if(hasMappingRow) {
    return {
      name: sheet.getName(),
      index: sheet.getIndex(),
      core: coreSheetIndex == sheet.getIndex(),
      rowType: props.getProperty(sheetKey + '-rowtype')
    };
  }
...
function getMappedSheets() {
    ...
    for(var i = 0; i < sheets.length; i++) {
      var metadata = getMetadata(sheets[i]);
      if (metadata != null) {
...
function setRowType(rowType, sheetIndex) {
  var props = PropertiesService.getDocumentProperties();
  var sheets = SpreadsheetApp.getActiveSpreadsheet().getSheets();
  if (sheetIndex > 0 && sheetIndex <= sheets.length) {
    var sheetKey = sheets[sheetIndex-1].getName().replace(/\s+/gi, '-').toLowerCase();
    props.setProperty(sheetKey+'-rowtype', rowType);
  }
}
...
function getDwCArchiveSchema() {
  ...
  for (var i = mappedSheets.length - 1; i >= 0; i--) {
    ...
    var isCore = mappedSheets[i].index == coreSheetIndex;
    var node = {
      id: sheet.getIndex(),
      label: sheet.getName(),
      size: 10,
      color: isCore ? '#f00' : '#000',
      x:  isCore ? 0 : x,
      y: isCore ? 0 : y,
      type: 'image',
      data: isCore ?
